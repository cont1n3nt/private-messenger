...
from app.tests.helpers import make_user_data


class TestCreateUser:
    async def test_create_user_success(self, session):
        u = await create_user(session, make_user_data("alice"))
    ...
    async def test_create_user_persists(self, session):
        u = await create_user(session, make_user_data("bob"))
        await session.commit()

        found = await get_user_by_id(session, u.id)
...
class TestCreateMultipleUsers:
    async def test_create_multiple_users(self, session):
        ...
        users = await create_multiple_users(session, users_data)
        await session.commit()
...
class TestGetUserById:
    ...
    async def test_get_nonexistent_user(self, session):
        found = await get_user_by_id(session, 999999)
        assert found is None


class TestGetUserByUsername:
    async def test_get_existing_user(self, session, user):
        found = await get_user_by_username(session, user.username)
        assert found is not None
        assert found.username == user.username

    async def test_get_nonexistent_user(self, session):
        found = await get_user_by_username(session, "nonexistent")
        assert found is None
...
class TestDeleteUser:
    ...
    async def test_delete_nonexistent_user(self, session):
        await delete_user(session, 999999)
...
class TestUpdateUserKeys:
    async def test_update_keys(self, session, user):
        ...
        updated = await update_user_keys(
            session, user.id,
    ...
    async def test_update_nonexistent_user(self, session):
        result = await update_user_keys(session, 999999, sign_public_key=b"key")
